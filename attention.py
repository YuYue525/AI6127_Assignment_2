...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden, encoder_hiddens):
        ...
        # attention
        attention_scores = torch.matmul(encoder_hiddens, hidden.reshape(-1, 1))
        prob_distribution = F.softmax(attention_scores, dim = 0).reshape(1, -1)
        attention = torch.matmul(prob_distribution, encoder_hiddens).reshape(1, 1, -1)

        output = self.softmax(self.out(torch.cat((hidden[0], attention[0]), dim = 1)))
        return output, hidden
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_hiddens = torch.zeros(input_length, encoder.hidden_size, device=device) # record all the hiddens
